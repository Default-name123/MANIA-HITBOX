...
class Controls extends FlxActionSet
{
	...
	case SIX:
		inline forEachBound(Control.S0, (action, state) -> addbutton(action, hitbox.k1, state));
		inline forEachBound(Control.S1, (action, state) -> addbutton(action, hitbox.k2, state));
		inline forEachBound(Control.S2, (action, state) -> addbutton(action, hitbox.k3, state));
		inline forEachBound(Control.S4, (action, state) -> addbutton(action, hitbox.k4, state));
		inline forEachBound(Control.S5, (action, state) -> addbutton(action, hitbox.k5, state));
		inline forEachBound(Control.S6, (action, state) -> addbutton(action, hitbox.k6, state));									
	case SEVEN:	
		inline forEachBound(Control.S0, (action, state) -> addbutton(action, hitbox.k1, state));
		inline forEachBound(Control.S1, (action, state) -> addbutton(action, hitbox.k2, state));
		inline forEachBound(Control.S2, (action, state) -> addbutton(action, hitbox.k3, state));
		inline forEachBound(Control.S3, (action, state) -> addbutton(action, hitbox.k4, state));
		inline forEachBound(Control.S4, (action, state) -> addbutton(action, hitbox.k5, state));
		inline forEachBound(Control.S5, (action, state) -> addbutton(action, hitbox.k6, state));
		inline forEachBound(Control.S6, (action, state) -> addbutton(action, hitbox.k7, state));
	case EIGHT:
